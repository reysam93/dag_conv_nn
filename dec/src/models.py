...
class Model():
    ...
    def __init__(self, arch, loss=torch.nn.MSELoss(reduction='mean'), device='cpu'):
        ...
        self.DAGNN_arch = isinstance(arch, DAGNN)
        self.DVAE_arch = isinstance(arch, DVAE)
    ...
    def _get_loss(self, X, Y, GSO):
        ...
        with torch.no_grad():
            if self.DAGNN_arch:
                Y = torch.stack([data.y.to(self.dev) for data in X])
                X = self.arch._collate_fn(X)
                Y_hat = self.arch.encode(X)
            elif self.DVAE_arch:
                Y = torch.stack([data['y'].to(self.dev) for data in X])
                X = self.arch._collate_fn(X)
                Y_hat = self.arch(X)
    ...
    def _train_batches(self, train_dl, GSO, iters, optimizer):
        ...
        for _ in range(iters):
            if self.DAGNN_arch:
                for Xb in train_dl:
                    Yb = torch.stack([data.y.to(self.dev) for data in Xb])
                    Xb = self.arch._collate_fn(Xb)
                    Y_hat = self.arch.encode(Xb)
    ...
    def fit(self, X, Y, GSO, lr, n_epochs, bs=100, wd=0, optim=torch.optim.Adam, eval_freq=10,
        ...
        if self.DAGNN_arch :
            pyg_data_train = [data.to(self.dev) for data in X['train']]
            X_val = [data1.to(self.dev) for data1 in X['val']]
            X_test = [data2.to(self.dev) for data2 in X['test']]
            Y_val = Y['val'].to(self.dev)
            Y_test = Y['test'].to(self.dev)
            train_dl = [pyg_data_train[i:i+bs] for i in range(0, len(pyg_data_train), bs)]
        elif self.DVAE_arch :
            X_val = X['val']
            X_test = X['test']
            Y_val = Y['val'].to(self.dev)
            Y_test = Y['test'].to(self.dev)
            train_dl = [X['train'][i:i+bs] for i in range(0, len(X['train']), bs)]
        else:
    ...
    def test(self, X, Y, GSO, arc):
        if arc == DAGNN:
            X = [data.to(self.dev) for data in X]
            Y = Y.to(self.dev)
            X = self.arch._collate_fn(X)
            Y_hat = self.arch.encode(X)
        elif arc == DVAE :
