...
class ParallelMLPSum(nn.Module):
    def __init__(self, n_inputs, input_dim, hidden_dims, output_dim):
        ...
        # NOTE: the number of inputs should match the number of GSOs
        self.n_inputs = n_inputs
        ...
        # self.n_params = 0
        # Calculate number of parameters once during initialization
        self.n_params = sum(p.numel() for p in self.mlps.parameters())
    ...
    def forward(self, inputs, GSO):
        ...
        # NOTE: shifting the signals wrt GSO could be done offline
        # NOTE: seems that we proces signals 1 by 1, could be done by batches? THe goal is to avoid the for loops
        ...
        # print('bacth', inputs.shape)
        # print('x_i', x_i.shape)
        # print('outputs', outputs[-1].shape, 'len', len(outputs))
        # print('u', u[-1].shape, 'len', len(u))

        # print('out', torch.stack(u).shape)
        # print('out', torch.stack(u).unsqueeze(1).shape)
        # print('out', torch.stack(u).unsqueeze(2).shape)
