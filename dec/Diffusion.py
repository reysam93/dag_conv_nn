...
data_p = {
    ...
    'max_src_node': 20, #25,           # Maximum index of nodes allowed to be sources
    ...
    'min_GSO': 20,               # Minimum index of GSOs involved in the diffusion
...
def run_exp(d_p, d_arc_args, d_mod_p, exps, verb=True):
    ...
    with tqdm(total=d_p['n_tries']*len(exps), disable=False) as pbar:
        for i in range(d_p['n_tries']):
            ...
            # Adj, W, GSOs, Psi, G = get_graph_data2(d_p, get_Psi=True)
            # X_data, Y_data, sel_GSOs, sel_GSOs_idx = get_signals(d_p, GSOs)
            ...
            for j, exp in enumerate(exps):
                ...
                if exp['arc_p']['arch'] == LinDAGRegModel:
                    ...
                    X_data1, Y_data1 = DVAE_model(Adj, X_data, Y_data)
...
Exps = [
    # Our Models 
    {'arc_p': {'arch': SMLP , 'GSO': 'GSOs', 'in_dim': 1, 'hid_dim': [501], 'out_dim': 1, 'bias' : True }, 'leg': 'SMLP-501'},
    {'arc_p': {'arch': SMLP , 'GSO': 'GSOs', 'in_dim': 1, 'hid_dim': [1024], 'out_dim': 1, 'bias' : True }, 'leg': 'SMLP-1024'},
    {'arc_p': {'arch': SMLP , 'GSO': 'GSOs', 'in_dim': 1, 'hid_dim': [256], 'out_dim': 1, 'bias' : True }, 'leg': 'SMLP-256'},

    # {'arc_p': {'arch': ParallelMLPSum , 'GSO': 'GSOs', 'n_inputs': 50, 'input_dim': 50, 'hidden_dims': [501,501], 'output_dim': 50}, 'leg': 'ParallelMLPSum'},
    # {'arc_p': {'arch': SharedMLPSum , 'GSO': 'GSOs', 'n_inputs': 50, 'input_dim': 50, 'hidden_dims': [501,501], 'output_dim': 50}, 'leg': 'SharedMLPSum'},
    ...
    #            'out_pool': P_MAX, 'dropout': 0, 'num_nodes': 100}, 'leg': 'DAGNN'},
    # {'arc_p': {'arch': DVAE, 'GSO': 'GSOs','max_n':100, 'nvt':1, 'START_TYPE':0, 'END_TYPE':1, 'hs':501,'nz':100, 'bidirectional': True, 'vid': True}, 'leg': 'DVAE'},
