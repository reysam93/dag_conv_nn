...
P_SUM = "add"  # do not use "sum" so that can be used to call tg pooling function
P_MAX = "max"
P_ATTN = "attn"
...
class DAGNN(nn.Module):
    ...
    def __init__(self, emb_dim, hidden_dim,
                 num_rels=2, w_edge_attr=False, num_layers=2, bidirectional=True, mapper_bias=True,  # bias only for DVAE simulation
                 agg_x=False, agg="attn_h", out_wx=False, out_pool_all=True, out_pool=P_MAX, encoder=None, dropout=0.0,
                 word_vectors=None, emb_dims=[], activation=None, num_class=2, recurr=1,
                 in_feat=1):
        super().__init__()
        ...
        self.out_hidden_dim = emb_dim + self.hidden_dim * len(self.dirs) * num_layers if out_wx else self.hidden_dim * len(self.dirs) * num_layers  # USING UNIFY*len(self.dirs)

        # self.out_dim = out_dim  # not needed in OGB

        # initial embedding
        self.encoder = nn.Linear(in_features=in_feat, out_features=emb_dim,bias=False)
    ...
    def forward(self, G):
        ...
        # print(G.bi_layer_idx0, G.bi_layer_index0)
        ...
        # print(G.bi_layer_index,G.bi_layer_index.shape)
            ...
            for l_idx in range(num_layers_batch):
                ...
                for i, cell in enumerate(self.__getattr__("cells_{}".format(d))):
                    if l_idx == 0:
                        ...
                        ps_h = node_agg(G.h[d][i], lp_edge_index, edge_attr=None, **kwargs)[layer]
